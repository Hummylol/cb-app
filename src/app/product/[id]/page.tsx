            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card className="shadow-sm border-slate-200 overflow-hidden">
                <CardHeader className="pb-3 bg-slate-50/50">
                  <div className="flex items-center justify-between">
                    ...
                    <a
                      ...
                      className="text-[10px] font-bold text-blue-600 hover:text-blue-700 underline flex items-center"
                    >
                      OPEN MAPS
                    </a>
                  </div>
                ...
                <CardContent className="pt-0">
                  ...
                  <div className="mt-3 flex items-center text-[10px] text-slate-400">
                    <div className="h-1.5 w-1.5 rounded-full bg-green-500 mr-2 shadow-[0_0_8px_rgba(34,197,94,0.6)]" />
              ...
              <Card className="shadow-sm border-slate-200 overflow-hidden">
                ...
                <CardContent className="pt-0">
                  {product.upi_qr ? (
                    <div className="flex items-center space-x-4">
                      <div className="h-14 w-14 rounded-lg border border-slate-200 p-1 bg-white shadow-inner overflow-hidden flex-shrink-0">
                        <img src={product.upi_qr} alt="UPI QR" className="h-full w-full object-contain" />
                      </div>
                      <div>
                        <p className="text-xs font-bold text-slate-700">Scan & Pay Online</p>
                        <p className="text-[10px] text-slate-500 mt-0.5">UPI ID verified via seller</p>
                        <button
                          onClick={() => {
                            ...
                            modal.innerHTML = `
                              <div class="bg-white p-6 rounded-2xl max-w-sm w-full animate-in fade-in zoom-in duration-300">
                                <div class="flex justify-between items-center mb-4">
                                  <h3 class="font-bold text-slate-900">Payment QR</h3>
                                  <span class="text-xs text-slate-400">Tap to close</span>
                                </div>
                                <img src="${product.upi_qr}" class="w-full aspect-square object-contain border border-slate-100 rounded-xl shadow-sm" />
                                <div class="mt-4 p-3 bg-slate-50 rounded-lg border border-slate-100">
                                  <p class="text-[11px] text-center text-slate-500 leading-tight">Please include the <b>product name</b> in your transaction notes for faster verification.</p>
                                </div>
                              </div>
                            `;
                            document.body.appendChild(modal);
                          }}
                          className="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 underline mt-1"
                        >
                          ENLARGE QR
                        </button>
                      </div>
                    ...
                    <div className="flex items-center space-x-3 text-slate-400">
                      ...
                      <p className="text-xs font-medium italic">Cash / UPI on physical pickup</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                ...
                <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center">
                      ...
                      <p className="text-sm font-semibold text-slate-900">{product.stock_quantity > 0 ? 'Ready to Rent' : 'Unavailable'}</p>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Seller Contact</span>
                    <div className="flex items-center">
                      <Phone className="h-3 w-3 mr-2 text-slate-400" />
                      <p className="text-sm font-semibold text-slate-900">{product.contact_number}</p>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Date Listed</span>
                    <p className="text-sm font-semibold text-slate-900">{new Date(product.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                  </div>
                </div>
                {product.additional_details && (
                  <div className="pt-4 border-t border-slate-100">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Important Notes</span>
                    <p className="text-sm font-medium text-slate-700 mt-2 leading-relaxed">{product.additional_details}</p>
                  </div>
                )}
              </CardContent>
            </Card>
