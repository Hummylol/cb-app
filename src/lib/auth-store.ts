    ...
    (set, get) => ({
      ...
      signUp: async (email, password, role) => {
        try {
          ...
          // In a "no email verification" setup, Supabase automatically creates the session
          // However, we still need to manage the user profile.
          ...
          // If insert fails due to duplicate key, the profile already exists (likely from a trigger)
          // We can just proceed as the trigger should have set the default role 'buyer'
          // We might want to update it to the selected role if it was different
          if (insertError) {
            if (insertError.code === '23505' || insertError.message?.includes('duplicate key')) {
              ...
              set({ loading: false })
              return { error: new Error('Database table "user_profiles" not found.') }
            } else {
